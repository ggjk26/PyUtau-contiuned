...
std::string normalizeAlias(std::string value) {
    value = trim(toLowerAscii(std::move(value)));
...
bool parseOtoFile(const std::filesystem::path& otoPath,
    ...
    while (std::getline(input, line)) {
        ...
        if (!entry.alias.empty()) {
            ...
            const auto normalized = normalizeAlias(entry.alias);
            if (!normalized.empty()) {
...
const OtoEntry* Voicebank::lookup(const std::string& lyric) const {
    if (const auto found = m_dictionary.find(lyric); found != m_dictionary.end()) {
        return &found->second;
    }

    const auto normalized = normalizeAlias(lyric);
    if (normalized != lyric) {
        if (const auto found = m_dictionary.find(normalized); found != m_dictionary.end()) {
            return &found->second;
        }
    }

    const auto withDash = "-" + normalized;
    if (const auto found = m_dictionary.find(withDash); found != m_dictionary.end()) {
        return &found->second;
    }
