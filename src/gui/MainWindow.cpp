...
void MainWindow::exportWav() {
    ...
    const auto render = m_resampler.renderTrack(m_project.tracks().front(), m_voicebank);
    if (!m_audio.exportWav(path.toStdString(), render.pcm)) {
        QMessageBox::critical(this, "Export", "Failed to write wav file.");
        return;
    }

    m_statusLabel->setText(QString("Exported: %1 | render %2 ms | threads %3")
                              .arg(path)
                              .arg(QString::number(render.elapsedMs, 'f', 2))
                              .arg(render.workerThreads));
}
