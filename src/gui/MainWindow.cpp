...
void MainWindow::buildUi() {
    ...
    connect(m_trackList, &QWidget::customContextMenuRequested, this, [this](const QPoint& pos) {
        QMenu menu(this);
        menu.addAction("Add Track", this, &MainWindow::addTrack);
        auto* removeTrack = menu.addAction("Remove Selected Track");
        QAction* chosen = menu.exec(m_trackList->mapToGlobal(pos));
        if (chosen == removeTrack && m_trackList->currentRow() >= 0 && m_trackList->currentRow() < static_cast<int>(m_project.tracks().size())) {
            m_project.tracks().erase(m_project.tracks().begin() + m_trackList->currentRow());
            if (m_project.tracks().empty()) {
                m_project.createTrack("Vocal #1");
            }
            bindProjectToUi();
            m_statusLabel->setText("Track removed");
        }
    });

    auto* pianoPanel = new QWidget(splitter);
    auto* pianoLayout = new QVBoxLayout(pianoPanel);
    pianoLayout->setContentsMargins(0, 0, 0, 0);
    pianoLayout->setSpacing(4);

    auto* pianoTools = new QWidget(pianoPanel);
    auto* pianoToolsLayout = new QHBoxLayout(pianoTools);
    pianoToolsLayout->setContentsMargins(0, 0, 0, 0);

    auto* toolPointer = new QToolButton(pianoTools);
    auto* toolDraw = new QToolButton(pianoTools);
    auto* toolPitch = new QToolButton(pianoTools);
    toolPointer->setText("Pointer");
    toolDraw->setText("Draw");
    toolPitch->setText("Pitch");
    toolPointer->setCheckable(true);
    toolDraw->setCheckable(true);
    toolPitch->setCheckable(true);
    toolPointer->setChecked(true);

    connect(toolPointer, &QToolButton::clicked, this, [=]() { toolPointer->setChecked(true); toolDraw->setChecked(false); toolPitch->setChecked(false); m_statusLabel->setText("Tool: Pointer"); });
    connect(toolDraw, &QToolButton::clicked, this, [=]() { toolPointer->setChecked(false); toolDraw->setChecked(true); toolPitch->setChecked(false); m_statusLabel->setText("Tool: Draw"); });
    connect(toolPitch, &QToolButton::clicked, this, [=]() { toolPointer->setChecked(false); toolDraw->setChecked(false); toolPitch->setChecked(true); m_statusLabel->setText("Tool: Pitch"); });

    auto* playBtn = new QToolButton(pianoTools);
    auto* pauseBtn = new QToolButton(pianoTools);
    auto* replayBtn = new QToolButton(pianoTools);
    playBtn->setText("Play");
    pauseBtn->setText("Pause");
    replayBtn->setText("Replay");
    connect(playBtn, &QToolButton::clicked, this, [this]() { m_statusLabel->setText("Transport: play preview."); });
    connect(pauseBtn, &QToolButton::clicked, this, [this]() { m_statusLabel->setText("Transport: paused."); });
    connect(replayBtn, &QToolButton::clicked, this, [this]() { m_statusLabel->setText("Transport: replay from start."); });

    auto* snapBox = new QComboBox(pianoTools);
    snapBox->addItem("1/4", 480);
    snapBox->addItem("1/8", 240);
    snapBox->addItem("1/16", 120);
    snapBox->addItem("1/32", 60);
    snapBox->setCurrentIndex(2);

    auto* tempoSpin = new QSpinBox(pianoTools);
    tempoSpin->setRange(40, 300);
    tempoSpin->setValue(120);

    pianoToolsLayout->addWidget(toolPointer);
    pianoToolsLayout->addWidget(toolDraw);
    pianoToolsLayout->addWidget(toolPitch);
    pianoToolsLayout->addSpacing(12);
    pianoToolsLayout->addWidget(playBtn);
    pianoToolsLayout->addWidget(pauseBtn);
    pianoToolsLayout->addWidget(replayBtn);
    pianoToolsLayout->addSpacing(12);
    pianoToolsLayout->addWidget(new QLabel("Snap", pianoTools));
    pianoToolsLayout->addWidget(snapBox);
    pianoToolsLayout->addWidget(new QLabel("Tempo", pianoTools));
    pianoToolsLayout->addWidget(tempoSpin);
    pianoToolsLayout->addStretch(1);

    m_pianoRoll = new PianoRollWidget(pianoPanel);
    m_pianoRoll->setContextMenuPolicy(Qt::CustomContextMenu);

    auto* scrollArea = new QScrollArea(pianoPanel);
    scrollArea->setWidgetResizable(false);
    scrollArea->setWidget(m_pianoRoll);

    connect(snapBox, &QComboBox::currentIndexChanged, this, [this, snapBox]() {
        m_pianoRoll->setSnapTick(snapBox->currentData().toInt());
        m_statusLabel->setText(QString("Snap changed: %1").arg(snapBox->currentText()));
    });
    connect(tempoSpin, &QSpinBox::valueChanged, this, [this](int tempo) {
        const int pixelsPerQuarter = std::clamp(tempo, 40, 300);
        m_pianoRoll->setPixelsPerQuarter(pixelsPerQuarter);
        m_statusLabel->setText(QString("Tempo changed: %1 BPM").arg(tempo));
    });
    ...
    connect(m_pianoRoll, &QWidget::customContextMenuRequested, this, [this](const QPoint& pos) {
        ...
        auto* lyricAction = menu.addAction("Insert/Edit Lyric");

        const int noteIndex = m_pianoRoll->noteIndexAt(pos);
        if (noteIndex < 0) {
            deleteNoteAction->setEnabled(false);
            lyricAction->setEnabled(false);
        }

        QAction* chosen = menu.exec(m_pianoRoll->mapToGlobal(pos));
        if (!chosen) {
            return;
        }

        if (chosen == addNoteAction) {
            track.notes.push_back(m_pianoRoll->makeNoteAt(pos, 240));
            std::ranges::sort(track.notes, [](const pyutau::core::Note& a, const pyutau::core::Note& b) { return a.startTick < b.startTick; });
            m_pianoRoll->setTrack(&track);
            m_statusLabel->setText("Note added");
        } else if (chosen == deleteNoteAction && noteIndex >= 0 && noteIndex < static_cast<int>(track.notes.size())) {
            track.notes.erase(track.notes.begin() + noteIndex);
            m_pianoRoll->setTrack(&track);
            m_statusLabel->setText("Note deleted");
        } else if (chosen == lyricAction && noteIndex >= 0 && noteIndex < static_cast<int>(track.notes.size())) {
            bool ok = false;
            const QString text = QInputDialog::getText(this, "Insert Lyric", "Lyric", QLineEdit::Normal,
                                                       QString::fromStdString(track.notes[static_cast<std::size_t>(noteIndex)].lyric), &ok);
            if (ok && !text.isEmpty()) {
                track.notes[static_cast<std::size_t>(noteIndex)].lyric = text.toStdString();
                m_statusLabel->setText(QString("Lyric updated: %1").arg(text));
            }
        }
    });

    pianoLayout->addWidget(pianoTools);
    pianoLayout->addWidget(scrollArea, 1);
