...
- **自由音素器选择（Phonemizer）**：音素器设置已迁移到“音轨设置”，每条音轨可独立选择 `JP VCV & CVVC（默认）`、`DiffSinger Japanese`、`PyUTAU Native`。
- **多音素器高负载警告**：当同次导出中启用的音素器类型达到 3 种及以上时，会弹出“速度缓慢/可能崩溃”警告；支持“我已知晓，继续”“放弃生成”与“不再弹出”。
